    ...
    try {
        ...
        if (user) {
            ...
            if (user.flags && user.flags.bitfield) {
                ...
                const flagNames = [];
                if (raw & 1) flagNames.push('STAFF');
                if (raw & 2) flagNames.push('PARTNER');
                if (raw & 4) flagNames.push('HYPESQUAD');
                if (raw & 8) flagNames.push('BUG_HUNTER');
                if (raw & 16) flagNames.push('MFA_SMS');
                if (raw & 32) flagNames.push('PREMIUM_PROMOTION_DISMISSED');
                if (raw & 64) flagNames.push('HOUSE_BRAVERY');
                if (raw & 128) flagNames.push('HOUSE_BRILLIANCE');
                if (raw & 256) flagNames.push('HOUSE_BALANCE');
                if (raw & 512) flagNames.push('PREMIUM_EARLY_SUPPORTER');
                if (raw & 1024) flagNames.push('TEAM_PSEUDO_USER');
                if (raw & 2048) flagNames.push('INTERNAL_APPLICATION');
                if (raw & 4096) flagNames.push('SYSTEM');
                if (raw & 8192) flagNames.push('HAS_UNREAD_URGENT_MESSAGES');
                if (raw & 16384) flagNames.push('BUG_HUNTER_LEVEL_2');
                if (raw & 32768) flagNames.push('UNDERAGE_DELETED');
                if (raw & 65536) flagNames.push('VERIFIED_BOT');
                if (raw & 131072) flagNames.push('VERIFIED_DEVELOPER');
                if (raw & 262144) flagNames.push('CERTIFIED_MODERATOR');
                if (raw & 524288) flagNames.push('BOT_HTTP_INTERACTIONS');
                if (raw & 1048576) flagNames.push('SPAMMER');
                if (raw & 2097152) flagNames.push('DISABLE_PREMIUM');
                if (raw & 4194304) flagNames.push('ACTIVE_DEVELOPER');
